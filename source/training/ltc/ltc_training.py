...
from scipy.stats import kendalltau  # Added for Kendall's tau

# Import LTC components from ncps
from ncps.torch import LTC

from training.lstm.lstm_eval import compare_position_errors, compare_orientation_errors, compare_velocity_errors, \
    evaluate_test_set, PerformanceVisualizer
...
class MetricsTracker:
    ...
    def update(self, outputs, targets):
        ...
        # Replace R² calculation with Kendall's tau
...
def train_ltc(X, y, device):
    ...
    # Initialize criterion and scheduler outside the training loop
    ...
    for comp in component_metrics:
        logger.info(f"Component {comp['component']}: "
                    f"RMSE={comp['rmse']:.4f}, "
                    f"τ={comp['kendall_tau']:.4f}, "  # Changed from 'r2' to 'kendall_tau'
                    f"MAE={comp['mae']:.4f}")
    ...
    for group_name, metrics in group_metrics.items():
        logger.info(f"{group_name.capitalize()}: "
                    ...
                    f"τ={metrics['kendall_tau']:.4f}, "  # Changed from 'r2' to 'kendall_tau'
...
def run_model_comparison(enhanced_model, test_loader, device):
    """
    Run unit-based comparison between LTC-enhanced model and standard Kalman filter
    """
    logger.info("Running unit-based model comparison...")
    ...
    logger.info(f"2D Error - Standard KF: {position_metrics['baseline_2d_error_m']:.4f}m, "
                f"LTC-Enhanced: {position_metrics['enhanced_2d_error_m']:.4f}m, "
                f"Improvement: {position_metrics['2d_improvement_percent']:.2f}%")

    logger.info(f"Height Error - Standard KF: {position_metrics['baseline_height_error_m']:.4f}m, "
                f"LTC-Enhanced: {position_metrics['enhanced_height_error_m']:.4f}m, "
                f"Improvement: {position_metrics['height_improvement_percent']:.2f}%")

    logger.info("\nOrientation Error Comparison (degrees):")
    logger.info(f"Roll Error - Standard KF: {orientation_metrics['baseline_roll_error_deg']:.4f}°, "
                f"LTC-Enhanced: {orientation_metrics['enhanced_roll_error_deg']:.4f}°, "
    ...
    logger.info(f"Pitch Error - Standard KF: {orientation_metrics['baseline_pitch_error_deg']:.4f}°, "
                f"LTC-Enhanced: {orientation_metrics['enhanced_pitch_error_deg']:.4f}°, "
    ...
    logger.info(f"Yaw Error - Standard KF: {orientation_metrics['baseline_yaw_error_deg']:.4f}°, "
                f"LTC-Enhanced: {orientation_metrics['enhanced_yaw_error_deg']:.4f}°, "
    ...
    logger.info(f"East Velocity Error - Standard KF: {velocity_metrics['baseline_east_vel_error_mps']:.4f}m/s, "
                f"LTC-Enhanced: {velocity_metrics['enhanced_east_vel_error_mps']:.4f}m/s, "
    ...
    logger.info(f"North Velocity Error - Standard KF: {velocity_metrics['baseline_north_vel_error_mps']:.4f}m/s, "
                f"LTC-Enhanced: {velocity_metrics['enhanced_north_vel_error_mps']:.4f}m/s, "
    ...
    logger.info(f"Up Velocity Error - Standard KF: {velocity_metrics['baseline_up_vel_error_mps']:.4f}m/s, "
                f"LTC-Enhanced: {velocity_metrics['enhanced_up_vel_error_mps']:.4f}m/s, "
                f"Improvement: {velocity_metrics['up_vel_improvement_percent']:.2f}%")

    # Create visualizations
    visualizer = PerformanceVisualizer()
    visualizer.plot_position_error_comparison(position_metrics, title_suffix='_ltc_final')
    visualizer.plot_orientation_error_comparison(orientation_metrics, title_suffix='_ltc_final')
    visualizer.plot_velocity_error_comparison(velocity_metrics, title_suffix='_ltc_final')
