...
def calculate_kendall_tau(y_true, y_pred):
    ...
    try:
        ...
        print(f"Error calculating Kendall's tau: {e}")
        return 0.0
...
def calculate_group_metrics(y_true, y_pred):
    ...
    for name, indices in groups.items():
        ...
        group_metrics[name] = {
            ...
            'kendall_tau': calculate_kendall_tau(group_true, group_pred)  # Replaced r2 with kendall_tau
        }
...
class PerformanceVisualizer:
    ...
    def plot_prediction_analysis(self, y_true, y_pred, title_suffix=''):
        ...
        for i, (group_name, components) in enumerate(component_groups.items()):
            ...
            # Add RMSE and Kendall's tau values as text
            text_str = "Metrics:\n"
            for comp_idx, comp_name in components.items():
                rmse = np.sqrt(mean_squared_error(y_true[:, comp_idx], y_pred[:, comp_idx]))
                tau = calculate_kendall_tau(y_true[:, comp_idx], y_pred[:, comp_idx])  # Changed from r2_score
                text_str += f"{comp_name}:\nRMSE={rmse:.4f}, τ={tau:.4f}\n\n"
    ...
    def plot_group_metrics(self, group_metrics, title_suffix=''):
        ...
        metrics_to_plot = ['rmse', 'mae', 'kendall_tau']  # Changed from 'r2' to 'kendall_tau'
        ...
        for i, metric in enumerate(metrics_to_plot):
            ...
            if metric == 'kendall_tau':
                plt.title(f"Kendall's τ by Component Group")
    ...
    def plot_position_error_comparison(self, metrics, title_suffix=''):
        ...
        rects2 = ax.bar(x + width / 2, enhanced_values, width, label='LSTM-Enhanced KF')
...
def evaluate_test_set(model, test_loader, device):
    """
    Main evaluation function with Kendall's tau replacing R²
    """
    model.eval()
    all_targets = []
    all_predictions = []
    ...
    # Basic metrics with Kendall's tau
    metrics = {
        'mse': mean_squared_error(y_true, y_pred),
        'rmse': np.sqrt(mean_squared_error(y_true, y_pred)),
        'mae': mean_absolute_error(y_true, y_pred),
        'kendall_tau': calculate_kendall_tau(y_true, y_pred),  # Replaced R²
    ...
    # Group metrics with Kendall's tau
    group_metrics = calculate_group_metrics(y_true, y_pred)

    # Component metrics with Kendall's tau for your thesis table
    components = {
    ...
    component_metrics = [
        {
            ...
            'kendall_tau': calculate_kendall_tau(y_true[:, i], y_pred[:, i]),  # Replaced R²
