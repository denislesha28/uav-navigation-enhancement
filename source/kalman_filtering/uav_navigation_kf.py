...
class UAVNavigationKF:
    ...
    def correct_nominal_state(self):
        ...
        if velocity_correction.ndim > 1:
            print(f"Warning: nominal_state has shape {self.nominal_state.shape}, flattening to 1D")
            velocity_correction = velocity_correction.flatten()
        ...
        if position_correction.ndim > 1:
            print(f"Warning: position correction has shape {position_correction.shape}, flattening to 1D")
            position_correction = position_correction.flatten()
        ...
        if np.all(np.isfinite(gyro_bias_error)):
            if gyro_bias_error.ndim > 1:
                print(f"Warning: position correction has shape {gyro_bias_error.shape}, flattening to 1D")
                gyro_bias_error = gyro_bias_error.flatten()
            self.gyro_bias += damping * gyro_bias_error

        accel_bias_error = self.kf.x[12:15].copy()
        if np.all(np.isfinite(accel_bias_error)):
            if accel_bias_error.ndim > 1:
                print(f"Warning: position correction has shape {accel_bias_error.shape}, flattening to 1D")
                accel_bias_error = accel_bias_error.flatten()
